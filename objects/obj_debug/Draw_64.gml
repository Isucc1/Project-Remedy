if room != rm_mainMenu{
	...
	if instance_exists(obj_playerMain){
		...
		str[0] = string("Class: {0}", global.class)
		str[1] = string("Movement Speed: {0} \nDelta Adjusted Movement Speed: {1}, \nx: {2}, \ny: {3}", global.playerSpeed, (global.deltaMultiplier*global.playerSpeed), player.x, player.y)
		str[2] = string("Player direction: {0}", player.directionFacing)
		str[3] = string("Max HP: {0}\nAtk: {1}\nDef: {2} (not yet implemented)\nSpeed: {3} pixel(s)/second\nFiring Speed: {4} second(s)\nRange: {5} pixels", global.playerMaxHealth, global.playerAttack, global.playerDefense, global.playerSpeed, global.playerFiringSpeed, global.playerRange)
		...
		draw_text_ext(0, 0, str[currentMessage], 12, 600)
	}
}